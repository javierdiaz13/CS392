 ...
 * Date        : 06/25/2020
 * Description : Sums number in an array.
 ...
 */
 //takes in struct
 //not increasing directly in the thread so that we dont have to lock and unlock the variable - so
 //that critical sections is not large
void *sum_array(void *ptr) {
	thread_args *ptr2 =(thread_args*)(ptr);
	int partial_sum = 0;
	for(int i = ptr2->start_index; i < ptr2->length; i = i + 2){
		 partial_sum = array[i] + partial_sum;	    
	}
	int retval;
...
int main(int argc, char *argv[]) {
    ...
    if ((retval = pthread_create(&threads[0], NULL, sum_array, (void *)(&targs[0]))) != 0) {
    ...
    if ((retval = pthread_create(&threads[1], NULL, sum_array, (void *)(&targs[1]))) != 0) {
